...
class ResNetFc_OfficeHome(nn.Module):
    def __init__(self, resnet_name, bottleneck_dim=256, class_num=1000):
        super(ResNetFc_OfficeHome, self).__init__()
...
class ResNetFc_ImageNetCaltech(nn.Module):
  def __init__(self, resnet_name, bottleneck_dim=256, class_num=1000,
               init_fc=0, NoRelu=0):
    super(ResNetFc_ImageNetCaltech, self).__init__()
    model_resnet = resnet_dict[resnet_name](pretrained=True)
    self.conv1 = model_resnet.conv1
    self.bn1 = model_resnet.bn1
    self.relu = model_resnet.relu
