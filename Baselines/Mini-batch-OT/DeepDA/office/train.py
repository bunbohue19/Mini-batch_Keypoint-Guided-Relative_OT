...
def build_list_path_candidates(raw_path, list_path):
    ...
    if office_home_root:
        ...
        if raw_path.startswith(OFFICE_HOME_LIST_PREFIX):
            candidates.append(osp.join(office_home_root, raw_path[len(OFFICE_HOME_LIST_PREFIX) :]))

    if raw_path.startswith(OFFICE_HOME_LIST_PREFIX):
        candidates.append(osp.join(base_dir, "images", raw_path[len(OFFICE_HOME_LIST_PREFIX) :]))
