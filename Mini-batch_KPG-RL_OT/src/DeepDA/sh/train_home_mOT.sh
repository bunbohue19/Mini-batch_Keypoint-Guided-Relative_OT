#!/bin/bash
# Office-Home  |  standard mini-batch OT  (averaging scheme, no BoMb, no KPG)
#
# Usage:
#   cd Mini-batch_KPG-RL_OT/src/DeepDA/sh
#   bash train_home_mOT.sh
#
# Tasks: all 12 Office-Home transfers  (Art / Clipart / Product / Real_World)
...
for ENTRY in "${TASK_LIST[@]}"; do
    ...
    OUTPUT_DIR="home_${TASK}_${METHOD}_k${K}_m${M}_eps${EPSILON}_run${RUN_ID}"
